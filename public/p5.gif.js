...
(function(root, factory) {
  ...
  var parseGIF = function(st, handler) {
    handler || (handler = {});
    print('actual parsing');

    // LZW (GIF-specific)
    var parseCT = function(entries) { // Each entry is 3 bytes, for RGB.
      print('parsing CT: entries = ' + entries);
      var ct = [];
      for (var i = 0; i < entries; i++) {
        ct.push(st.readBytes(3));
      }
      print('finished parsing CT');
      return ct;
    };
  ...
  var SuperGif = function(opts) {
    ...
    var init = function() {
      pGraphics = options.p5inst.createImage(1, 1);
      tmpCanvas = pGraphics.canvas;
      print('made weird graphics');
	...
	print('load setup did not fail soo....');
	  ...
	  console.log('we got an error');
        ...
        h.open('GET', src, true);
        h.send();
	print('finished registering h functions');
      },
